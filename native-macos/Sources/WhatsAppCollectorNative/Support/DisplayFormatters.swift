...
enum DisplayFormatters {
    static func date(_ value: String?) -> String {
        guard let value, let date = ISO8601DateFormatter().date(from: value) else { return "Never" }
    ...
    static func relativeDate(_ value: String?) -> String {
        guard let value, let date = ISO8601DateFormatter().date(from: value) else { return "No export" }
